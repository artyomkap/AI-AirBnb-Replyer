...
async def send_message_to_unread(page: Page, chat_ids: List[str], message_text: str):
    for chat_id in chat_ids:
        try:
            ...
            await page.wait_for_load_state("networkidle")
            logger.info("✅ Состояние networkidle")

            await page.wait_for_timeout(2000)
            logger.info("⏱️ Дождались timeout")
            ...
            # ⏳ Ожидание поля ввода
            await page.wait_for_selector("textarea#message_input", timeout=10000)
            ...
            try:
                ...
                with open(f"output/debug_chat_{chat_id}.html", "w", encoding="utf-8") as f:
                    f.write(html_debug)
            except:
                pass
            continue
...
async def scheduler(interval_seconds: int = 30):
    first_run = True
    try:
        while True:
